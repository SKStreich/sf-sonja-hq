...
-- captures: add org_id if a prior migration didn't, then gate on it
ALTER TABLE captures ADD COLUMN IF NOT EXISTS org_id UUID REFERENCES orgs(id);
DROP POLICY IF EXISTS "captures_tenant_isolation" ON captures;
CREATE POLICY "captures_tenant_isolation" ON captures FOR ALL
  USING (org_id = get_my_org_id());

-- focus_notes: same idempotent org_id guard
ALTER TABLE focus_notes ADD COLUMN IF NOT EXISTS org_id UUID REFERENCES orgs(id);
