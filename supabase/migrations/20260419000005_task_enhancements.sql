-- Add 'cancelled' to task_status enum. `status` is an enum column (not a
-- text+CHECK), so the prior DROP/ADD CONSTRAINT approach was a no-op locally
-- and failed because 'cancelled' wasn't a valid enum label yet.
