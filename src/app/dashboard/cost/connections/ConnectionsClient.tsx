...
export function ConnectionsClient({ initial, backupState = [] }: { initial: ServiceConfig[]; backupState?: BackupStateRow[] }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <BackupStatusSection state={backupState} />
...
function BackupStatusRow({ row }: { row: BackupStateRow }) {
  ...
  const statusBadge = status === 'success'
    ...
    : status === 'partial'
      ? <span className="rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-amber-800">⚠ Partial</span>
      : status === 'error'
  ...
  return (
    <li className="px-4 py-3">
      <div className="flex items-center gap-3">
        <span className="text-sm font-medium text-gray-900">{row.bucket_name}</span>
        {statusBadge}
      ...
      {(row.objects_synced_last_run > 0 || row.objects_skipped_last_run > 0) && (
