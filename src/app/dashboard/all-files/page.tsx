  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-8">
      <div className="mb-6">
        ...
        <h1 className="mt-2 text-2xl font-bold text-white">All Files</h1>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {files.map((f: any, i: number) => (
                <tr
                  key={f.id}
                  className={`${i < files.length - 1 ? 'border-b border-gray-800/50' : ''} hover:bg-gray-900/40 transition-colors`}
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span className="text-base shrink-0">📎</span>
                      <span className="text-white font-medium truncate max-w-xs">{f.filename}</span>
                    </div>
                  </td>
