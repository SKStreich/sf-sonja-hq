...
// Simple SVG bar chart — last 30 days, stacked by service
...
function AddEntryForm({ onDone }: AddEntryFormProps) {
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div>
          ...
          <input type="number" step="0.01" min="0" value={cost} onChange={e => setCost(e.target.value)}
            placeholder="0.00"
            className="w-full rounded-lg border border-gray-700 bg-gray-900 px-3 py-2 text-sm text-white outline-none placeholder-gray-600" />
        </div>
        <div>
...
export function CostDashboard({ usage, serviceConfig }: { usage: UsageRow[]; serviceConfig: ServiceConfig }) {
  ...
  // Per-service MTD
  const byService: Record<string, { cost: number; units: number; rows: UsageRow[] }> = {}
  for (const row of mtd) {
    if (!byService[row.service]) byService[row.service] = { cost: 0, units: 0, rows: [] }
    ...
    <div className="mx-auto max-w-4xl px-6 py-8">
              ...
              {usage.slice(0, 50).map((row: UsageRow) => {
                ...
                return (
                  <tr key={row.id} className="border-b border-gray-800/30 hover:bg-gray-900/20 transition-colors">
                    ...
                    <td className="px-4 py-2">
                      <span className={`text-xs px-1.5 py-0.5 rounded ${row.source === 'manual' ? 'bg-gray-800 text-gray-500' : row.source === 'auto' ? 'bg-indigo-900/40 text-indigo-400' : 'bg-green-900/30 text-green-500'}`}>
                        {row.source}
                      </span>
                    </td>
                    <td className="px-4 py-2">
      ...
      {usage.length === 0 && !addingEntry && (
        <div className="rounded-2xl border border-dashed border-gray-800 py-16 text-center">
          ...
          <p className="text-xs text-gray-700 mb-4">Add a manual entry or use the Sync button to pull from OpenAI</p>
