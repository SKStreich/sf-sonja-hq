...
const SERVICE_META: Record<string, { label: string; icon: string; color: string; unitLabel: string; pricingNote: string }> = {
  anthropic: { label: 'Anthropic / Claude',  icon: '🤖', color: 'text-purple-600',  unitLabel: 'tokens',   pricingNote: '$3/M input · $15/M output' },
  openai:    { label: 'OpenAI / Whisper',    icon: '🎙', color: 'text-green-600',   unitLabel: 'minutes',  pricingNote: '$0.006/min' },
  supabase:  { label: 'Supabase',            icon: '🗄',  color: 'text-emerald-600', unitLabel: 'requests', pricingNote: 'Free · Pro $25/mo' },
  vercel:    { label: 'Vercel',              icon: '▲',   color: 'text-gray-900',    unitLabel: 'deploys',  pricingNote: 'Hobby free · Pro $20/mo' },
  netlify:   { label: 'Netlify',             icon: '◈',   color: 'text-teal-600',    unitLabel: 'builds',   pricingNote: 'Starter free · Pro $19/mo' },
  resend:    { label: 'Resend',              icon: '✉',   color: 'text-blue-600',    unitLabel: 'emails',   pricingNote: '$0.80/1k emails' },
  other:     { label: 'Other',               icon: '⚙',   color: 'text-gray-600',    unitLabel: 'units',    pricingNote: '' },
}
        ...
        return (
          <g key={d}>
            <rect x={x} y={height - barH} width={barW} height={barH} rx={2}
              fill={d === today ? '#6366f1' : '#e5e7eb'} opacity={val > 0 ? 1 : 0.5} />
            {(i === 0 || i === 14 || i === 29) && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div>
          ...
          <input type="number" step="0.01" min="0" value={cost} onChange={e => setCost(e.target.value)}
            placeholder="0.00" className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-400" />
        </div>
        <div>
          <label className="text-xs text-gray-500 uppercase tracking-wider block mb-1">Units (optional)</label>
          <input type="number" value={units} onChange={e => setUnits(e.target.value)}
            placeholder="e.g. 1000" className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-400" />
        </div>
      </div>
      <div className="flex items-center gap-3">
        <input value={notes} onChange={e => setNotes(e.target.value)} placeholder="Notes (e.g. April invoice)"
          className="flex-1 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 outline-none placeholder-gray-400" />
        <button onClick={onDone} className="text-xs text-gray-500 hover:text-gray-700 transition-colors">Cancel</button>
...
export function CostDashboard({ usage, serviceConfig, serviceConfigs }: Props) {
  ...
  const isIdle = (svc: string): boolean => {
    ...
    if (!last) return serviceConfig[svc] ?? false
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <Link href="/dashboard" className="text-xs uppercase tracking-wider text-gray-400 hover:text-gray-600 transition-colors">← Dashboard</Link>
      ...
      {syncResults && (
        <div className="mb-6 rounded-xl border border-gray-200 bg-white px-4 py-3">
          <p className="text-xs font-medium uppercase tracking-wider text-gray-500 mb-2">Last Sync Results</p>
          <div className="flex flex-wrap gap-3">
            {syncResults.map(r => (
              <div key={r.service} className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-gray-600 capitalize">{r.service}:</span>
                {r.error === 'Paused'
                  ...
                  : <span className="text-xs text-green-600">{r.synced} records</span>
                }
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div key={c.label} className="rounded-xl border border-gray-200 bg-white px-5 py-4">
            <p className="text-xs font-medium uppercase tracking-wider text-gray-500 mb-2">{c.label}</p>
      ...
      <div className="rounded-xl border border-gray-200 bg-white px-5 py-4 mb-6">
        ...
        <p className="text-xs text-gray-400 mt-1 text-right">Today highlighted in indigo</p>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 overflow-hidden mb-6">
        <div className="border-b border-gray-200 bg-gray-50 px-4 py-3">
          ...
          <p className="text-xs text-gray-400 mt-0.5">Pause a service to skip it during sync. Auto-logging always resumes when the service is used.</p>
        </div>
        {TRACKED_SERVICES.map(svc => {
          ...
          return (
            <div key={svc} className="flex items-center gap-4 px-4 py-3 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
              <span className="text-lg w-6 shrink-0 text-center">{meta.icon}</span>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <p className={`text-sm font-medium ${meta.color}`}>{meta.label}</p>
                  {idle && status === 'active' && (
                    <span className="text-xs px-1.5 py-0.5 rounded bg-amber-100 text-amber-700">Idle</span>
                  )}
                  {status === 'paused' && (
                    <span className="text-xs px-1.5 py-0.5 rounded bg-gray-100 text-gray-500">Paused</span>
                  )}
                </div>
                <p className="text-xs text-gray-400">{meta.pricingNote}</p>
                <p className="text-xs text-gray-400 mt-0.5">
                  {configured ? (
                    ...
                    <span className="text-amber-600">API key not configured</span>
                  )}
                </p>
              </div>
              {data ? (
                <>
                  <div className="text-right shrink-0">
                    <p className="text-sm font-semibold text-gray-900">{fmtCost(data.cost)}</p>
                    <p className="text-xs text-gray-500">{data.units > 0 ? `${data.units.toFixed(1)} ${meta.unitLabel}` : `${data.rows.length} entr${data.rows.length !== 1 ? 'ies' : 'y'}`}</p>
                  </div>
                  {mtdTotal > 0 && (
                    <div className="w-20 h-1.5 rounded-full bg-gray-200 shrink-0">
                      <div className="h-1.5 rounded-full bg-indigo-600" style={{ width: `${Math.min((data.cost / Math.max(mtdTotal, 0.01)) * 100, 100)}%` }} />
                    </div>
                  )}
                </>
              ) : (
                <span className="text-xs text-gray-400 shrink-0 mr-1">No data</span>
              )}
              <button
                ...
                className={`shrink-0 rounded-lg border px-3 py-1 text-xs font-medium transition-colors disabled:opacity-40 ${
                  status === 'active'
                    ...
                    : 'border-amber-200 text-amber-600 hover:border-green-300 hover:text-green-600'
                }`}
              >
        ...
        {byService['other'] && (() => {
          ...
          return (
            <div key={svc} className="flex items-center gap-4 px-4 py-3 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-medium ${meta.color}`}>{meta.label}</p>
                {meta.pricingNote && <p className="text-xs text-gray-400">{meta.pricingNote}</p>}
              </div>
              <div className="text-right shrink-0">
                <p className="text-sm font-semibold text-gray-900">{fmtCost(data.cost)}</p>
                <p className="text-xs text-gray-500">{data.units > 0 ? `${data.units.toFixed(1)} ${meta.unitLabel}` : `${data.rows.length} entr${data.rows.length !== 1 ? 'ies' : 'y'}`}</p>
              ...
              {usage.slice(0, 50).map((row: UsageRow) => {
                ...
                return (
                  <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-2 text-xs text-gray-500">{row.metric_type}</td>
                    <td className="px-4 py-2 text-xs font-mono text-gray-900">{fmtCost(Number(row.cost_usd))}</td>
                    <td className="px-4 py-2">
                      <span className={`text-xs px-1.5 py-0.5 rounded ${
                        row.source === 'manual' ? 'bg-gray-100 text-gray-500'
                        : row.source === 'auto' ? 'bg-indigo-100 text-indigo-700'
      ...
      {usage.length === 0 && !addingEntry && (
        <div className="rounded-2xl border border-dashed border-gray-200 py-16 text-center">
          ...
          <p className="text-xs text-gray-400 mb-4">Click Sync All to pull data from connected services, or add a manual entry</p>
          <div className="flex items-center justify-center gap-3">
            <button onClick={handleSyncAll} disabled={syncing}
              className="rounded-lg border border-gray-200 px-4 py-2 text-sm text-gray-500 hover:text-gray-700 hover:border-gray-400 transition-colors">
              {syncing ? 'Syncing…' : '↻ Sync All'}
            </button>
