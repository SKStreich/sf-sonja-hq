...
const UPDATE_TYPE_CONFIG: Record<string, { label: string; color: string; dot: string }> = {
  note:      { label: 'Note',      color: 'text-gray-500',   dot: 'bg-gray-400' },
  progress:  { label: 'Progress',  color: 'text-green-600',  dot: 'bg-green-500' },
  blocker:   { label: 'Blocker',   color: 'text-red-600',    dot: 'bg-red-500' },
  decision:  { label: 'Decision',  color: 'text-indigo-600', dot: 'bg-indigo-500' },
  milestone: { label: 'Milestone', color: 'text-amber-600',  dot: 'bg-amber-500' },
}
...
export function ProjectDetail({ project, tasks: initialTasks, updates: initialUpdates, files: initialFiles, entity, entities, initialCommits = [], initialGithubUrl = null, members = [] }: Props) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const saved = await saveProjectFile(project.id, {
        filename: file.name,
        storage_path: path,
        file_size: file.size,
        content_type: file.type,
      })
      if (saved) setFiles(fs => [saved as ProjectFile, ...fs])
      router.refresh()
    } catch (err) {
  ...
  const inputCls = 'w-full rounded-lg bg-white px-3 py-2 text-sm text-gray-900 ring-1 ring-gray-200 focus:ring-indigo-400 outline-none transition-all placeholder:text-gray-400'
  ...
  return (
    <>
      <div className="mx-auto max-w-3xl px-6 py-8">
        ...
        {/* Breadcrumb */}
        <div className="flex items-center gap-2 text-xs text-gray-400 mb-6 uppercase tracking-wider">
          <Link href="/dashboard/projects" className="hover:text-gray-600 transition-colors">Projects</Link>
          <span>/</span>
        ...
        <div className="flex items-start justify-between gap-4 mb-2">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-2">
              ...
              {project.phase && (
                <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-500">{project.phase}</span>
              )}
            </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button onClick={() => setEditOpen(true)}
              className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium uppercase tracking-wider text-gray-600 hover:text-gray-900 hover:border-gray-400 transition-colors">
              Edit
            </button>
            <Link href={`/dashboard/projects/${project.id}/print`} target="_blank"
              className="rounded-md border border-gray-200 px-3 py-1.5 text-xs text-gray-500 hover:text-gray-700 hover:border-gray-400 transition-colors">
              Print
            </Link>
            <button onClick={handleArchive} disabled={archiving}
              className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium uppercase tracking-wider text-gray-400 hover:text-red-600 hover:border-red-200 transition-colors">
              Archive
            </button>
          </div>
        </div>
        ...
        {/* Due date */}
        {project.due_date && (
          <p className={`text-xs mb-4 ${projectOverdue ? 'text-red-500' : 'text-gray-500'}`}>
        ...
        {/* Next Action */}
        {project.next_action && (
          <div className={`rounded-xl px-4 py-3 mb-6 ${nextActionOverdue ? 'bg-red-50 border border-red-200' : 'bg-indigo-50 border border-indigo-200'}`}>
            <div className="flex items-center justify-between gap-3 mb-1">
              <div className="flex items-center gap-2">
                <p className={`text-xs font-medium uppercase tracking-wider ${nextActionOverdue ? 'text-red-600' : 'text-indigo-600'}`}>
                  Next Action
                </p>
              ...
              {(project as any).next_action_due && (
                <span className={`text-xs ${nextActionOverdue ? 'text-red-500 font-medium' : 'text-gray-500'}`}>
                  {nextActionOverdue ? '⚠ ' : ''}{formatDate((project as any).next_action_due)}
                </span>
              )}
            </div>
            <p className="text-sm text-gray-700">{project.next_action}</p>
          </div>
        )}

        {/* Tabs */}
        <div className="flex items-center gap-1 mb-5 border-b border-gray-200 pb-3">
          <button className={tabCls('tasks')} onClick={() => setActiveTab('tasks')}>
            Tasks <span className="ml-1 text-gray-400">{tasks.length}</span>
          </button>
          <button className={tabCls('log')} onClick={() => setActiveTab('log')}>
            Notes <span className="ml-1 text-gray-400">{updates.length}</span>
          </button>
          <button className={tabCls('files')} onClick={() => setActiveTab('files')}>
            Files <span className="ml-1 text-gray-400">{files.length}</span>
          </button>
            ...
            <form onSubmit={handleAddUpdate} className="mb-5 space-y-2">
              <div className="flex gap-2">
                <select value={updateType} onChange={e => setUpdateType(e.target.value)}
                  className="rounded-lg bg-white px-3 py-2 text-xs font-medium uppercase tracking-wider text-gray-600 ring-1 ring-gray-200 focus:ring-indigo-400 outline-none transition-all shrink-0">
                ...
                {updates.map(update => {
                  ...
                  return (
                    <div key={update.id} className="group relative">
                      <div className={`absolute -left-[1.3125rem] top-1.5 w-2.5 h-2.5 rounded-full border-2 border-white ${cfg.dot}`} />
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <button onClick={() => handleDeleteUpdate(update.id)}
                          className="opacity-0 group-hover:opacity-100 text-gray-300 hover:text-red-500 transition-all text-base leading-none shrink-0 mt-0.5">
                          ×
                        </button>
                      </div>
                    </div>
            ...
            <div className="mb-6">
              ...
              {githubEditing ? (
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setGithubEditing(false)} className="text-xs text-gray-500 hover:text-gray-700 transition-colors">Cancel</button>
                </div>
              ) : githubUrl ? (
                <div className="flex items-center gap-3">
                  <a href={githubUrl} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1.5 text-sm text-gray-600 hover:text-gray-900 transition-colors font-mono truncate">
                    {githubUrl.replace('https://github.com/', '')} ↗
                  </a>
                  <button onClick={() => setGithubEditing(true)} className="text-xs text-gray-400 hover:text-gray-600 shrink-0 transition-colors">Edit</button>
                </div>
              ) : (
                ...
                <div className="space-y-0.5">
                  {commits.map(commit => (
                    <a key={commit.sha} href={commit.url} target="_blank" rel="noopener noreferrer"
                      ...
                      <span className="mt-0.5 font-mono text-xs text-gray-400 shrink-0 w-14 truncate">{commit.sha.slice(0, 7)}</span>
                      <span className="flex-1 text-sm text-gray-700 group-hover:text-gray-900 transition-colors leading-snug">{commit.message.split('\n')[0]}</span>
                      <div className="shrink-0 text-right">
                        <p className="text-xs text-gray-400">{commit.author}</p>
                        <p className="text-xs text-gray-400">{new Date(commit.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</p>
                      </div>
                    </a>
                  ))}
                </div>
          ...
          <div>
            <div className="mb-5">
              ...
              <button onClick={() => fileInputRef.current?.click()} disabled={uploading}
                className="flex items-center gap-2 rounded-lg border border-gray-200 px-4 py-2.5 text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-400 disabled:opacity-50 transition-colors">
                {uploading ? (
                  <>
              ...
              <p className="mt-1.5 text-xs text-gray-400">PDF, Word, Excel, images, CSV, ZIP · max 50 MB</p>
            </div>
                  ...
                  <div key={file.id}
                    ...
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleDownload(file)}
                        className="text-xs text-gray-500 hover:text-indigo-600 transition-colors font-medium uppercase tracking-wider">
                        Download
                      </button>
